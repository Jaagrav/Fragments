const createDoc = (parentDir) => {
    ...
    const doc = `<div class="doc ${docID}" onclick="createTab('${docID}');openEditor('${docID}')">
        ...
        <input type="text" value="untitled" readonly="false" spellcheck="false" ondblclick="this.readOnly=false; this.focus();" onblur="this.readOnly=true;if(this.value==='') {event.path[1].remove();document.querySelector('.tabs .tab.${docID}').remove();}" onkeypress="if(event.key === 'Enter') {this.readOnly=true;if(this.value==='') {event.path[1].remove();document.querySelector('.tabs .tab.${docID}').remove();}this.blur();}" onkeyup="if(this.value==='' && this.readOnly) {event.path[1].remove();document.querySelector('.tabs .tab.${docID}').remove();} document.querySelector('.tabs .tab.${docID} span').innerText=this.value; this.setAttribute('value',this.value);" />
        <div class="options">
