const createDir = (parentDir) => {
    const dir = `<div class="dir-wrapper">
        <div class="dir-name" onclick="this.nextSibling.nextSibling.classList.toggle('open');this.children[0].classList.toggle('fa-folder');this.children[0].classList.toggle('fa-folder-open');">
            ...
            <input type="text" value="untitled" readonly="true" spellcheck="false" ondblclick="this.readOnly=false; this.focus();" onblur="this.readOnly=true;if(this.value==='') event.path[1].remove();" onkeypress="if(event.key === 'Enter') this.readOnly=true;" onkeyup="this.setAttribute('value',this.value);if(this.value==='' && this.readOnly) event.path[1].remove();" />
            <div class="options">
